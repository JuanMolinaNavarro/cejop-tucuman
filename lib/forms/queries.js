...
export async function getDashboardData() {
  ...
  const vacio = {
    ...
    porDia: [],
    porEncuentro: [],
    demografia: demografiaVacia,
  };
  if (!hasSupabaseEnv()) return vacio;
  const supabase = await createClient();

  const [encRes, formRes, respRes, acredRes, pregRes] = await Promise.all([
    supabase.from("encuentros").select("*").order("numero", { ascending: true }),
    supabase.from("formularios").select("id, encuentro_id, tipo, estado"),
    supabase
      .from("respuestas")
      .select("id, encuentro_id, formulario_id, created_at, respuestas"),
    supabase.from("acreditaciones").select("id, encuentro_id"),
    supabase.from("preguntas").select("id, formulario_id, etiqueta, tipo"),
  ]);
  ...
  // Demografía agregada de TODOS los encuentros (edad + localidad). Se detecta la
  // pregunta de edad/localidad de cada formulario de inscripción por su etiqueta.
  const edadPregPorForm = {};
  const locPregPorForm = {};
  for (const p of preguntas) {
    if (tipoPorForm[p.formulario_id] !== "inscripcion") continue;
    if (edadPregPorForm[p.formulario_id] == null && esPreguntaEdad(p))
      edadPregPorForm[p.formulario_id] = p.id;
    if (locPregPorForm[p.formulario_id] == null && esPreguntaLocalidad(p))
      locPregPorForm[p.formulario_id] = p.id;
  }
  const edades = [];
  const locsInscriptos = [];
  for (const r of inscripciones) {
    const j = r.respuestas ?? {};
    const ep = edadPregPorForm[r.formulario_id];
    const lp = locPregPorForm[r.formulario_id];
    if (ep != null && j[ep] != null && j[ep] !== "") edades.push(j[ep]);
    if (lp != null && j[lp] != null && String(j[lp]).trim()) locsInscriptos.push(j[lp]);
  }
  const edadDist = distribucionNumerica(edades, 40);
  const locFrec = agruparTexto(locsInscriptos, { localidad: true });
  // Top 7 + "Otras" para mantener la torta legible (≤ 8 segmentos).
  const TOP_LOC = 7;
  const localidadDemo =
    locFrec.length > TOP_LOC + 1
      ? [
          ...locFrec.slice(0, TOP_LOC),
          {
            valor: "Otras",
            count: locFrec.slice(TOP_LOC).reduce((s, x) => s + x.count, 0),
            percent: locsInscriptos.length
              ? Math.round(
                  (locFrec.slice(TOP_LOC).reduce((s, x) => s + x.count, 0) /
                    locsInscriptos.length) *
                    100
                )
              : 0,
          },
  ...
  const demografia = {
    edad: edadDist,
    localidad: localidadDemo,
    totalEdad: edadDist.count,
    totalLocalidad: locsInscriptos.length,
  };
